...
void UShooterAnimInstance::UpdateAnimationProperties(float DeltaTime)
{
	...
	if (ShooterCharacter)
	{
		...
		bEqupping  = ShooterCharacter->GetCombatState() == ECombatState::ECS_Equipping;
		bShouldUseFABRIK = ShooterCharacter->GetCombatState() == ECombatState::ECS_Unoccupied
			|| ShooterCharacter->GetCombatState() == ECombatState::ECS_FireTimerInPrograss;
		...
		// Check if ShooterCharacter holds valid gun
		if (ShooterCharacter->GetEquppedWeapon())
		{
			EquippedWeaponType = ShooterCharacter->GetEquppedWeapon()->GetWeaponType();
		}
	}
	TurnInPlace();
...
void UShooterAnimInstance::TurnInPlace()
{
	...
	else // not turning in place
	{
		...
		{
			if (bAiming || bReloading || bEqupping)
			{
				RecoilWeigt = 1.f;
			}
			else
...
void UShooterAnimInstance::Lean(float DeltaTime)
{
	...
	if (GEngine)
	{
		GEngine->AddOnScreenDebugMessage(1, -1, FColor::Cyan, FString::Printf(TEXT("Delta.Yaw: %f"), Delta.Yaw));
	}
}
