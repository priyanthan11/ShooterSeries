...
void AItem::OnConstruction(const FTransform& Transform)
{
	...
	//Path to item Rarity Item raityTable
	...
	if (RarityTableObject)
	{
		...
		if (RarityRow)
		{
			...
			IconBackground = RarityRow->ItemBackground;
			if (GetItemMesh())
			{
	...
	if (MaterialInstance)
	{
		...
		DynamicMaterialInstance->SetVectorParameterValue(TEXT("FresnlColor"), GlowColor);
...
void AItem::UpdatePulse()
{
	...
	case EItemState::EIS_Pickup:
		...
		break;
	case EItemState::EIS_EquipInterp:
		if (InsterpPulseCurve)
		{
			ElapsedTime = GetWorldTimerManager().GetTimerElapsed(ItemInterpTimer);
			CurveValue = InsterpPulseCurve->GetVectorValue(ElapsedTime);
		}
		break;
	}
	if (DynamicMaterialInstance)
